    ...
    return Array.from({ length: amount * 13 }, (_, index) => {
      ...
      return {
        value:
          valueIndex <= 8
            ? 2 + valueIndex
            : royalCards[valueIndex - 9] === "A"
            ? 11
            : 10,
        name:
          valueIndex <= 8
            ? (2 + valueIndex).toString()
            : royalCards[valueIndex - 9],
        suit: suit,
      };
    });
  };
...
const isSoftHand = (hand) => {
  ...
  const totalWithAcesAsOne = handTotal(
    hand.map((card) => (card.name === "A" ? { ...card, value: 1 } : card))
  );
...
export const strategyCheck = (handType, dealerCardTotal, playerHand) => {
  ...
  const playerCardRanges = Object.keys(
    decisionMatrix[handType][dealerCardTotal]
  );
